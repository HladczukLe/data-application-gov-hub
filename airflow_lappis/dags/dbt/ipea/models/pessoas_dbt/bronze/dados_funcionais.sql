with
    dados_tratados as (
        select
            ...
            nullif(
                trim(codocorringressoservpublico), ''
            ) as cod_ocorr_ingresso_serv_publico,
            ...
            to_date(
                nullif(trim(dataexercicionoorgao), ''), 'DDMMYYYY'
            ) as dt_exercicio_no_orgao,
            to_date(nullif(trim(datafimvalear), ''), 'DDMMYYYY') as dt_fim_vale_ar,
            to_date(
                nullif(trim(dataingressofuncao), ''), 'DDMMYYYY'
            ) as dt_ingresso_funcao,
            to_date(
            ...
            nullif(
                trim(nomeocorringressoservpublico), ''
            ) as nome_ocorr_ingresso_serv_publico,
        ...
        from {{ source("siape", "dados_funcionais") }}
    )
...
from dados_tratados as df
left join {{ ref("dados_uorg") }} as du on df.sigla_uorg_exercicio = du.sigla_uorg
